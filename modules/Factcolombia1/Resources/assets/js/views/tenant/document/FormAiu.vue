<template>
    <div class="card mb-0 pt-2 pt-md-0">
        <div class="card-body" v-if="loading_form">
                                ...
                                <div class="form-group" :class="{ 'has-danger': errors.type_invoice_id }">
                                    ...
                                    <el-select @change="changeResolution" v-model="form.resolution_id"
                                        ...
                                        <el-option v-for="option in resolutions" :key="option.id" :value="option.id"
                                            :label="`${option.prefix} / ${option.description} / ${option.resolution_number} / ${option.from} / ${option.to}`"></el-option>
                                    </el-select>
                                    <small class="form-control-feedback" v-if="errors.type_invoice_id"
                                        v-text="errors.type_invoice_id[0]"></small>
                                </div>
            ...
            <document-form-item ref="documentFormItem" :showDialog.sync="showDialogAddItem" :recordItem="recordItem"
                :isEditItemNote="false" :operation-type-id="form.operation_type_id"
                :currency-type-id-active="form.currency_id" :currency-type-symbol-active="ratePrefix()"
...
export default {
    ...
    computed: {
        getTotalBase() {
            return this.form.items.filter(row => row.item.internal_id != 'aiu00001' && row.item.internal_id != 'aiu00002' && row.item.internal_id != 'aiu00003').reduce((p, c) => Number(p) + (Number(c.total)), 0)
        }
    },
    async created() {
    ...
    methods: {
            ...
            val.items.forEach(item => {
                ...
                if (item.tax != null) {
                    ...
                    if (item.tax.is_percentage)
                        item.total_tax = ((item.price * item.quantity - (item.discount < item.price * item.quantity ? item.discount : 0)) * (item.tax.rate / item.tax.conversion)).toFixed(2);
        ...
        getInvoiceLines() {
            ...
            let data = this.form.items.map(x => {
                const { code, description } = this.getDescriptionCode(x.item.internal_id, x.item.name)
                return {
        ...
        getDescriptionCode(internal_id, name) {
            let code = null
