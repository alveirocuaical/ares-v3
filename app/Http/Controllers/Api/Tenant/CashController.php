...
class CashController extends Controller
{
    ...
    public function storeRestaurant(Request $request) {
        ...
        foreach ($request->internalsId as $row) {
            if($row['type'] == 'POS'){
                ...
                // ]);
                $document = DocumentPos::where('external_id', $row['external_id'])->first();
    ...
    public function cash_document(Request $request) {
        ...
        $documentModel = $isDocument ? Document::class : DocumentPos::class;
        $documentField = $isDocument ? 'document_id' : 'document_pos_id';
        $paymentConditionField = $isDocument ? 'payment_form_id' : 'payment_method_id';
        $creditCondition = $isDocument ? '2' : '09';

        $document = $documentModel::findOrFail((int) $request->$documentField);

        // $isCredit = $document->$paymentConditionField === $creditCondition;
        $isCredit = $isDocument && $document->payment_form_id === '2';

        $cashDocumentCredit = !$isCredit ? CashDocument::create([
            'cash_id' => $cash->id,
            $documentField => $document->id,
        ]) : null;
        
        $cashDocument = $cash->cash_documents()->updateOrCreate([
            'document_pos_id' => $request->document_pos_id,
    ...
    public function close() {
        ...
        foreach ($cash->cash_documents as $cash_document) {
            ...
            if($cash_document->document_pos){
                ...
                if(in_array($cash_document->document_pos->state_type_id, ['01','03','05','07','13'])){
                ...
                if(in_array($cash_document->document->state_document_id, [1, 2, 3, 4, 5])){
